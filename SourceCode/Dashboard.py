...
class Dashboard(QtWidgets.QMainWindow):
    def __init__(self):
        ...
        self.label_3.setGeometry(105, 160, 780, 440)
        movie.start()
    ...
    def quitApplication(self):
        ...
        userReply = QMessageBox.question(self, 'Thoát', "Bạn có muốn thoát?",
    ...
    def createGest(self):
        ...
        try:
            ...
            pass
        self.exit_button.clicked.connect(self.quitApplication)
        self.plainTextEdit.setPlaceholderText("Ý nghĩa của cử chỉ")
        img_text = ''
        saveimg = []
        while True:
            ...
            try:
                ...
                img2 = cv2.rectangle(frame, (200, 50), (350, 200), (0, 255, 0), thickness=2, lineType=8, shift=0)
            except:
            ...
            upper_blue = np.array([255, 255, slider2])
            imcrop = img2[50:200, 200:350]
            ...
            cv2.setWindowProperty("mask", cv2.WND_PROP_FULLSCREEN, cv2.WINDOW_FULLSCREEN)
            cv2.resizeWindow("mask", 170, 170)
            cv2.moveWindow("mask", 1100, 375)
            # self.textBrowser_3.setPixmap(QPixmap.fromImage(mask))
        ...
        if os.path.exists("./SampleGestures/" + str(gesname) + ".png"):
            QtWidgets.QMessageBox.about(self, "Success", "Đã lưu!")
    ...
    def on_click(self):
        ...
        if not os.path.exists('temp.txt'):
            if os.path.exists('.txt'):
                ...
                QtWidgets.QMessageBox.about(self, "Information", "Đã lưu!")
    ...
    def scanSent(self):
        ...
        while True:
            ...
            try:
                ...
                frame = cv2.cvtColor(frame, cv2.COLOR_BGR2RGB)
                img = cv2.rectangle(frame, (200, 50), (350, 200), (0, 255, 0), thickness=2, lineType=8, shift=0)
            except:
                keyboard.press_and_release('esc')
                # keyboard.press_and_release('esc')
            ...
            lower_blue = np.array([50, 50, 50])
            upper_blue = np.array([255, 255, slider])
            imcrop = img[50:200, 200:350]
            hsv = cv2.cvtColor(imcrop, cv2.COLOR_BGR2HSV)
            ...
            cv2.setWindowProperty("mask", cv2.WND_PROP_FULLSCREEN, cv2.WINDOW_FULLSCREEN)
            cv2.resizeWindow("mask", 150, 150)
            cv2.moveWindow("mask", 1250, 370)
    ...
    def scanSingle(self):
        ...
        while True:
            ...
            try:
                frame = cv2.resize(frame, (400, 300))
                frame = cv2.cvtColor(frame, cv2.COLOR_BGR2RGB)
                img1 = cv2.rectangle(frame, (200, 50), (350, 200), (0, 255, 0), thickness=2, lineType=8, shift=0)
            except:
                keyboard.press_and_release('esc')
            ...
            lower_blue = np.array([50, 50, 50])
            upper_blue = np.array([255, 255, slider1])

            imcrop = img1[50:200, 200:350]
            hsv = cv2.cvtColor(imcrop, cv2.COLOR_BGR2HSV)
            mask = cv2.inRange(hsv, lower_blue, upper_blue)
            ...
            cv2.imshow("mask", mask)
            cv2.setWindowProperty("mask", cv2.WND_PROP_FULLSCREEN, cv2.WINDOW_FULLSCREEN)
            cv2.resizeWindow("mask", 140, 140)
            cv2.moveWindow("mask", 1209, 370)
